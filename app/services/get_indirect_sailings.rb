  ...
  def eligible_sailings
    ...
    sailings.each do |first_leg|
      if first_leg.origin_port == origin_port
        ...
        second_legs = sailings.select { |sailing| sailing.origin_port == intermediate_port && sailing.destination_port == destination_port }

        second_legs.each do |second_leg|
          indirect_sailings << [first_leg, second_leg]
        end
      end
    end
